...
" functions that are only occasionally used. The Vim-python3-plugin has
...
if v:version < 704
    call RWarningMsgInp("The Vim-python3-plugin requires Vim >= 7.4.")
...
function! CompleteChunkOptions()
    ...
    let ktopt = ["eval=;TRUE", "echo=;TRUE", "results=;'markup|asis|hold|hide'",
                ...
                \ "child=; ", "engine=;'python3'", "opts.label=;''", "purl=;TRUE",
                \ 'python3.options=; ']
    if &filetype == "rnoweb"
        ...
        if np == 0
            ...
            if string(g:SendCmdToR) != "function('SendCmdToR_fake')"
                ...
                if g:rplugin_vimcomport > 0
                    ...
                    if g:rplugin_lastev != "NOT_EXISTS" && g:rplugin_lastev != "NO_ARGS" && g:rplugin_lastev != "python3 is busy." && g:rplugin_lastev != "NOANSWER" && g:rplugin_lastev != "INVALID" && g:rplugin_lastev != "" && g:rplugin_lastev != "No reply"
                        let args = []
            ...
            " If python3 isn't running, use the prebuilt list of objects
...
function MovePosRLineComment(lnum, cpos)
    ...
    if &filetype == "rnoweb"
        ...
        if ok == 0
            call RWarningMsg("Not inside an python3 code chunk.")
            return
        endif
    endif
    ...
    if &filetype == "rhelp"
        ...
        if ok == 0
            call RWarningMsg("Not inside an python3 code section.")
            return
        endif
    endif
...
function StartR_TmuxSplit(rcmd)
    ...
    if !g:vimrplugin_restart
        " Let Tmux automatically kill the panel when python3 quits.
        let tcmd .= " '" . a:rcmd . "'"
    endif
...
function StartR_ExternalTerm(rcmd)
    if $DISPLAY == "" && !g:rplugin_is_darwin
        call RWarningMsg("Start 'tmux' before Vim. The X Window system is required to run python3 in an external terminal.")
        return
    endif
    ...
    if v:shell_error
        ...
        if g:rplugin_is_darwin
            call RWarningMsg("Tmux session with python3 is already running")
            return
        endif
        if g:rplugin_termcmd =~ "gnome-terminal" || g:rplugin_termcmd =~ "xfce4-terminal" || g:rplugin_termcmd =~ "terminal" || g:rplugin_termcmd =~ "iterm"
...
function IsSendCmdToRFake()
    if string(g:SendCmdToR) != "function('SendCmdToR_fake')"
        if exists("g:maplocalleader")
            call RWarningMsg("As far as I know, python3 is already running. Did you quit it from within Vim (" . g:maplocalleader . "rq if not remapped)?")
        else
            call RWarningMsg("As far as I know, python3 is already running. Did you quit it from within Vim (\\rq if not remapped)?")
        endif
        return 1
    endif
    return 0
...
function StartR(whatr)
    ...
    elseif v:servername == ""
        if has("gui_macvim") && !has("gui_running")
            call RWarningMsg("The Vim application that comes with MacVim cannot receive messages from python3.")
        endif
    ...
    let start_options += ['if(utils::packageVersion("vimcom") != "1.2.7") warning("Your version of Vim-python3-plugin requires vimcom-1.2-7.", call. = FALSE)']
    ...
    call writefile(start_options, g:rplugin_tmpdir . "/start_options.python3")
    ...
    if a:whatr =~ "custom"
        call inputsave()
        let b:rplugin_r_args = input('Enter parameters for python3: ')
        call inputrestore()
    endif
    ...
    " python3 was already started. Should restart it or warn?
    if string(g:SendCmdToR) != "function('SendCmdToR_fake')"
        if g:rplugin_do_tmux_split
            if g:vimrplugin_restart
                ...
                if IsExternalOBRunning()
                    ...
                    call SendToVimCom("\005G .GlobalEnv [Restarting python3]")
    ...
    if has("clientserver") && v:servername == "" && $DISPLAY != ""
        call RWarningMsgInp("You have to restart Vim with the --servername argument otherwise Vim cannot receive messages from python3.")
        redraw
...
" To be called by edit() in python3 running in Neovim buffer.
...
function WaitVimComStart()
    ...
    if filereadable(g:rplugin_tmpdir . "/vimcom_running_" . $VIMINSTANCEID)
        ...
        if g:rplugin_vimcom_version != "1.2.7"
            call RWarningMsg('This version of Vim-python3-plugin requires vimcom 1.2.7.')
            sleep 1
        endif
        call delete(g:rplugin_tmpdir . "/vimcom_running_" . $VIMINSTANCEID)

        if !has("libcall")
            call RWarningMsgInp("+libcall feature is missing: communication with python3 will be flawed.")
        endif

        if g:rplugin_do_tmux_split
            " Environment variables persists across Tmux windows.
            " Unset VIMRPLUGIN_TMPDIR to avoid vimcom loading its C library
            " when python3 was not started by Vim:
            call system("tmux set-environment -u VIMRPLUGIN_TMPDIR")
        endif
        call delete(g:rplugin_tmpdir . "/start_options.python3")

        " Vim may crash when the syntax highlight is changed while the window
        " is being resized, although only if the python3 syntax is included in
...
function RObjBrowser()
    " Only opens the Object Browser if python3 is running
    if string(g:SendCmdToR) == "function('SendCmdToR_fake')"
        call RWarningMsg("The Object Browser can be opened only if python3 is running.")
        return
    endif
...
function RBrOpenCloseLs_Vim(status)
    ...
    " Avoid possibly freezing cross messages between Vim and python3
...
function RFormatCode() range
    ...
    let g:rplugin_lastev = ReadEvalReply()
    if g:rplugin_lastev == "python3 is busy." || g:rplugin_lastev == "UNKNOWN" || g:rplugin_lastev =~ "^Error" || g:rplugin_lastev == "INVALID" || g:rplugin_lastev == "ERROR" || g:rplugin_lastev == "EMPTY" || g:rplugin_lastev == "No reply"
...
function SendCmdToR_fake(cmd)
    call RWarningMsg("Did you already start python3?")
    return 0
endfunction
...
function SendCmdToR_Term(cmd)
    ...
    " Send the command to python3 running in an external terminal emulator
    let str = substitute(cmd, "'", "'\\\\''", "g")
...
function RSourceLines(...)
    ...
    let rcmd = 'exec(open("' . g:rplugin_rsource . '"' . sargs . ').read())'
    let ok = g:SendCmdToR(rcmd)
    return ok
endfunction

" Send file to python3
function SendFileToR(e)
    ...
    call g:SendCmdToR('exec(open("' . fpath .  '"' . sargs . ').read())')
endfunction

" Send block to python3
...
" Send python3 code from the first chunk up to current line
...
function SetRTextWidth(rkeyword)
    ...
    if !bufloaded(s:rdoctitle) || g:vimrplugin_newsize == 1
        ...
        " s:vimpager is used to calculate the width of the python3 help documentation
...
" Show python3's help doc in Vim's buffer
...
function ShowRDoc(rkeyword)
    ...
    if a:rkeyword =~ "^MULTILIB"
        ...
        let flines = ['',
                    ...
                    \ 'Press <Enter> over one of them to see the python3 documentation:',
                    \ '']
    ...
    " If the help command was triggered in the python3 Console, jump to Vim pane
...
function RSetPDFViewer()
    if exists("g:vimrplugin_pdfviewer") && g:vimrplugin_pdfviewer != "none"
        ...
        elseif executable("zathura")
            ...
            " List from python3 configure script:
...
" Call python3 functions for the word under cursor
...
function RCreateStartMaps()
    " Start
    "-------------------------------------
    call RCreateMaps("nvi", '<Plug>RStart',        'rf', ':call StartR("python3")')
...
function RCreateEditMaps()
    ...
    if hasmapto("<Plug>RCompleteArgs", "i")
        inoremap <buffer><silent> <Plug>RCompleteArgs <C-python3>=RCompleteArgs()<CR>
    else
        inoremap <buffer><silent> <C-X><C-A> <C-python3>=RCompleteArgs()<CR>
    endif
endfunction
...
" ========================================================================
" Set default mean of communication with python3
...
if g:rplugin_is_darwin
    ...
    elseif isdirectory("/Applications/python3.app")
        call RSetDefaultValue("g:vimrplugin_applescript", 1)
    else
...
if !has("win32") && !has("win64") && !has("gui_win32") && !has("gui_win64") && !g:vimrplugin_applescript
    if !executable('tmux') && g:vimrplugin_source !~ "screenR"
        call RWarningMsgInp("Please, install the 'Tmux' application to enable the Vim-python3-plugin.")
        let g:rplugin_failed = 1
        finish
    endif
    ...
    if g:rplugin_tmux_version < "1.8" && g:vimrplugin_source !~ "screenR"
        call RWarningMsgInp("Vim-python3-plugin requires Tmux >= 1.8")
        let g:rplugin_failed = 1
        finish
    endif
endif
...
" Control the menu 'python3' and the tool bar buttons
if !exists("g:rplugin_hasmenu")
    let g:rplugin_hasmenu = 0
endif

" List of marks that the plugin seeks to find the block to be sent to python3
...
if g:vimrplugin_term == "gnome-terminal" || g:vimrplugin_term == "xfce4-terminal" || g:vimrplugin_term == "terminal" || g:vimrplugin_term == "lxterminal"
    ...
    if g:vimrplugin_vim_wd
        let g:rplugin_termcmd = g:vimrplugin_term . " --title python3 -e"
    else
        let g:rplugin_termcmd = g:vimrplugin_term . " --working-directory='" . expand("%:p:h") . "' --title python3 -e"
    endif
endif

if g:vimrplugin_term == "terminator"
    if g:vimrplugin_vim_wd
        let g:rplugin_termcmd = "terminator --title python3 -x"
    else
        let g:rplugin_termcmd = "terminator --working-directory='" . expand("%:p:h") . "' --title python3 -x"
    endif
endif
...
if g:vimrplugin_term == "roxterm"
    ...
    if g:vimrplugin_vim_wd
        let g:rplugin_termcmd = "roxterm --title python3 -e"
    else
        let g:rplugin_termcmd = "roxterm --directory='" . expand("%:p:h") . "' --title python3 -e"
    endif
endif
...
if g:vimrplugin_term == "rxvt" || g:vimrplugin_term == "urxvt"
    let g:rplugin_termcmd = g:vimrplugin_term . " -cd '" . expand("%:p:h") . "' -title python3 -xrm '*iconPixmap: " . g:rplugin_home . "/bitmaps/ricon.xbm' -e"
endif
...
elseif has("win32") || has("win64")
    ...
    let g:rplugin_R = "python3"
endif
if !executable(g:rplugin_R)
    call RWarningMsgInp("python3 executable not found: '" . g:rplugin_R . "'")
endif
